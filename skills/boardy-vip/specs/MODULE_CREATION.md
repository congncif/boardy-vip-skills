...
> Companion specs: `IO_INTERFACE.md` (IO layer details), `PLUGINS_INTEGRATION.md` (LauncherPlugin wiring), `{ProjectConfigPath}` (project-specific values).
...
Pick an available scheme and device from the output (or use `{Destination}` from `{ProjectConfigPath}` when present).
...
Find the app entry file that owns `UIWindow` (declared in `{ProjectConfigPath}`; commonly `SceneDelegate` for UIKit scene apps or `AppDelegate` for older UIKit apps).
...
1. Clones the module template repository configured for the project (for example: `{ModuleTemplateURL}` from `{ProjectConfigPath}`)
