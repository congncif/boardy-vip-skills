...
> Companion specs: `MICROBOARD_UI.md` (UI board variant), `COMMUNICATION.md` (flow + bus patterns), `EXAMPLES_NONUI_BOARDS.md` / `EXAMPLES_VIEWLESS_BOARD.md` (concrete skeletons).
...
Is it a coordinator/flow orchestrator (activates child boards in sequence)?
    ...
    (save a child's output to pass into / configure a later child)
        ...
        NO  → FlowBoard ← {ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Flow

Is it a single async operation where the CALLER needs to handle its result per-activation?
    YES → BlockTaskBoard ← {ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Block Task
...
Is it a single async operation where result goes to the motherboard output stream?
    ...
    One at a time (sequential)?        → TaskBoard  ← {ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Single Task
...
Need to block an action until a prerequisite completes (e.g. login before cart)?
    YES → BarrierBoard ← {ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Barrier

Fully custom logic that doesn't fit above?
    YES → Empty Board ← {ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Empty
```
...
**Template:** `{ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Block Task`
...
**Template:** `{ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Single Task`
...
**Template:** `{ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Single Result Task`
...
**Template:** `{ModuleTemplatesPath}/Templates/Non-UI Board.xctemplate/Barrier`
