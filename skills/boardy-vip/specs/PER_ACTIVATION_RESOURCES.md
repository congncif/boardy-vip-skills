...
> Companion specs: `MICROBOARD_NONUI.md` (Viewless Board), `COMMUNICATION.md` (Bus patterns, `complete()` semantics).
...
- `complete()` must be called only after ALL attached work is done — see `COMMUNICATION.md` for the full `complete()` semantics and when-to-call rules
