...
- [ ] LauncherPlugin wired in the app entry file declared by `{ProjectConfigPath}` via `.install(launcherPlugin:)` before `.initialize()`
...
- [ ] Read `COMPOSABLE_BOARD.md` for full rules (not duplicated here)
